      ...
      <div v-else class="buttons-col left"></div>
			...
			<Knob :disabled="!conditioner.isActive"
				...
				<div class="indicators hbox alic">
					...
					<BtnIcon icon="icon-powerlevel" :value="+conditioner.powerLevel" @click.native="cyclePowerlevels" />
				</div>
				<div class="temp-info">
...
export default {
  ...
  computed: {
    ...
    selectedPowerLevel () {
      return this.powerlevels[+this.conditioner.powerLevel]?.name
		...
		activePowerName() {
			...
			return item ? item.name : +this.conditioner.mode;
		},
    ...
    sensorDevice: {
      deep: true,
      handler(val) {
        const temp = hexToDecimal(val.status)
