...
import { hsvToHex, hexToDecimal } from "@/utils/transformers.js";
...
export default {
  ...
  async mounted() {
    ...
    this.sensorData = hexToDecimal(this.state)
  },
  destroyed() {
    ...
    sensorDevice: {
      ...
      handler(val) {
        if (this.addr === val.addr) {
          this.sensorData = hexToDecimal(val.status)
        }
      }
    }
  },
  computed: {
    ...
    ...mapActions('modules/settings', ['subscribeRequest', 'stopSubscribeRequest']),
		async handleClick() {
      if (this.item.__.icon === 'conditioner') return
      let status
      if (this.state === '00') {
        status = 1
